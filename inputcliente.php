<?php

    SESSION_START();
    if (isset($_SESSION["login"])){
        $usuario = $_SESSION["login"];
?>

<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Sessão de cliente</title>
    <link rel="shortcut icon" href="img/sneakers.png" type="image/x-icon">
    <link rel="stylesheet" href="styleinputs.css">
    <script src="https://use.fontawesome.com/c382bc2935.js"></script>
    <link rel="preconnect" href="https://fonts.gstatic.com">
</head>
<body>

<main class="container">
        <h2>Cadastro de cliente</h2>
        <form action="php/crud/inserirdadoscliente.php" method="post">
            <div class="input-field">
                <input type="text" name="cxnomecliente"
                placeholder="Nome e Sobrenome">
                <div class="underline"></div>
            </div>
            <div class="input-field">
                <input type="text" name="cxenderecocliente"
                placeholder="Endereço ">
                <div class="underline"></div>
            </div>
            <div class="input-field"><br>
                <input type="text" name="cxbairrocliente"
                placeholder="Bairro">
                <div class="underline"></div>
            </div>
            <div class="input-field"><br>
                <input type="text" name="cxcidadecliente"
                placeholder="Cidade">
                <div class="underline"></div>
            </div>
            <div class="input-field"><br>
                <input type="number" name="cxcepcliente"
                placeholder="CEP apenas números">
                <div class="underline"></div>
            </div>
            <div class="input-field"><br>
                <input type="number" name="cxtelefonecliente"
                placeholder="DDD+Número apenas números">
                <div class="underline"></div>
            </div>
            <div class="input-field"><br>
                <input type="email" name="emailcliente"
                placeholder="Email">
                <div class="underline"></div>
            </div>
            <div class="input-field"><br>
                <input type="number" name="idadecliente"
                placeholder="Idade">
                <div class="underline"></div>
            </div>
            <div class="input-field"><br>
                <input type="number" name="ncalcadocliente"
                placeholder="Nº Calçado">
                <div class="underline"></div>
            </div>

            <input type="submit" value="Cadastrar Cliente">
        </form>

        <div class="footer">
            <span>Busca avançada</span>
            <div class="social-field">
                <div class="social-field search">
                    <a href="php/crud/formbuscli.php">
                        <i class="fa fa-search" aria-hidden="true"></i>
                        Buscar Clientes
                    </a>
                </div>
                <div class="social-field list">
                    <a href="php/crud/listarcliente.php">
                        <i class="fa fa-list" aria-hidden="true"></i>
                        Listar clientes
                    </a>
                </div>
                <div class="social-field signout">
                    <a href="php/fimsessao.php">
                        <i class="fa fa-sign-out" aria-hidden="true"></i>
                        Sair
                    </a>
                </div>
            </div>
        </div>
    </main>

</body>
</html>

<?php 
}
else{
    echo "<script> 
            window.location.href = 'index.php';
        </script>
        ";
} 
?>
